 ...
 */
void
init_at91rm9200(unsigned channel, const char *init, const char *defaults)
{
	...
	out32(base + AT91RM9200_DBGU_IDR, 0x000F3FFF);

	if (baud != 0) {
		/* Reset Rx, Tx and Status bits */
		out32(base + AT91RM9200_DBGU_CR, AT91RM9200_DBGU_CR_RSTRX |
				AT91RM9200_DBGU_CR_RSTTX | AT91RM9200_DBGU_CR_RSTSTA);

		/* 8-bit, no-parity, 1 stop bit */
		out32(base + AT91RM9200_DBGU_MR, (3 << 6) | (4 << 9));

		/* Program BRG Register */
		out32(base + AT91RM9200_DBGU_BRGR, (clk >> 4) / baud);
	}

	/* Enable Tx/Rx */
	out32(base + AT91RM9200_DBGU_CR, AT91RM9200_DBGU_CR_RXEN |
			AT91RM9200_DBGU_CR_TXEN);
}
 ...
 */
void
put_at91rm9200(int data)
{
	unsigned base = dbg_device[0].base;

	while (!(in32(base + AT91RM9200_DBGU_CSR) & AT91RM9200_DBGU_IE_SR_TXRDY))
		;

	out32(base + AT91RM9200_DBGU_THR, (unsigned)data);
}
