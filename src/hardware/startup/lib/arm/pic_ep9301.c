 ...
 * The 2nd is for the 16 interruptable pins of GPIO ports A and B. This is a
...
static struct startup_intrinfo intrs[] =
{
	{	
		...
		.cpu_intr_stride = 0,
		.flags = 0,
		.id = {.genflags = 0, .size = 0, .rtn = &interrupt_id_ep93xx},
		.eoi = {.genflags = INTR_GENFLAG_LOAD_INTRMASK, .size = 0, .rtn = &interrupt_eoi_ep93xx},
 ...
 * connected to the processor. There is no documentation on how priorization
 ...
 * To properly support priotized interrupts and decouple them from the assigned
...
uint32_t ep9301_config_pic(uint32_t os_vector_base, uint32_t pic_vector_base, vector_tbl_t *reg_vectors)
{
	...
	*/
	ASSERT(valid_pic_id());
	ASSERT(pic_vector_base == os_vector_base);	// remapping not supported (yet)
	 ...
	 * some devices do actually have interrupts asserted and this is reflectedin the RAW register(s) */
	...
	*/
	...
	/* GPIO 
	 * 19 of the EP9301 GPIO ports are interruptable and are handled via a
