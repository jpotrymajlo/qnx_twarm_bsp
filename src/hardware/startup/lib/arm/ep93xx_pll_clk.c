 ...
 * </quote>
 * 
 * Since Cirrus has ignored repeated requests to explain what modes the bits
 * actually specify and which bit within CP15 register 1 these bits actually are,
 * I am going to make the following educated guess.
 * 
 * 		nF - when set indicates fast bus mode
 * 		iA - when set indicates asynchronous mode
 * 
 * For the purposes of selecting which clock is driving the processor, it appears
 * from the description above that only knowing whether Fast bus mode is set is
 * required and therefore, together with my guess on the meaning of the bits
 * 
 * 		if the nF bit is set (ie. fast bus mode), then the CPU uses HCLK
 * 		if the nF bit is clear then the CPU uses FCLK
 * 
 * Now if I only knew which bit nF was ...
 * 
 * A scan of the IPL that Bally wrote indicates that it set setting the
 * processor to Fast bus mode which also happens to be its default mode.
 * Additionally there are 2 defines, MMU_nf and MMU_iA which define the nF bit
 * as 30 and the iA bit as 31. I have no idea if these are correct.
 * 
 * It is possible however that the nF bit is bit 10 which is marked as
 * implementation defined in the ARM architecture reference manual. This bit
 * however is reading as 0 if without being modified by the IPL and therefore
 * its possible that it means FCLK. That is, if it's set, use the FCLK as the
 * cpu clock thus indicating NOT fast bus mode.
*/
#define nF_bit	(1U << 30)
...
//kprintf("CP15 reg 1, op 0 = 0x%x\n", CP15_reg1);
	return (CP15_reg1 & nF_bit) ? ep93xx_get_hclk() : ep93xx_get_fclk();
}
