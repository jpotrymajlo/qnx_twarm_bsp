    ...
    if (dev->rlen < dev->xlen) {
        ...
        case 1:
            ...
            if (dev->tlen < dev->xlen)
                out32(base + AT91_SPI_TXDATA, dev->pbuf[dev->tlen++] 
				                              | (dev->curr_cs<<SPI_TXDATA_PCS_POS));
            break;
        case 2:
            *(uint16_t *)(&dev->pbuf[dev->rlen]) = data;
             ...
             */
            if (dev->tlen < dev->xlen) {
                out32(base + AT91_SPI_TXDATA, (*(uint16_t *)(&dev->pbuf[dev->tlen]))
				                                | (dev->curr_cs<<SPI_TXDATA_PCS_POS));
                dev->tlen += 2;
            }
            break;
...
#else
    spi_sr_val=in32(base + AT91_SPI_STATREG);
    if(spi_sr_val & SPI_INTREG_RXBUFF){
	/* Disable receiver */
	out32 (base + PDC_PTCR, PTCR_RXTDIS | PTCR_TXTDIS );
	/* Disable buffer complete interrupt */
	out32(base + AT91_SPI_INTDISREG, SPI_INTREG_RXBUFF);
	return (&dev->spievent);}
#endif
